...
var appPopup = {
    ...
    add_handlers: function() {
        if (!app_item_ids instanceof Array || app_item_ids.length < 1 || !app_pic_paths instanceof Array || app_pic_paths.length < 1)
        {
            return false;
        }
    ...
    showPopupLink: function(event, popup_link_id) {
        if (!appPopup.is_popup_link_active)
        {
            ...
            YAHOO.util.Event.on(popup_link, "click", appPopup.displayPopup, app_pic_paths[index]);
        }
    },
    ...
    displayPopup: function(event, pic_path) {
        ...
        if (img && appPopup.is_popup_link_active)
        {
            img.src = pic_path;
